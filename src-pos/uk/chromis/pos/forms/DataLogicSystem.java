...
public class DataLogicSystem extends BeanFactoryDataSingle {
    ...
    public void init(Session s) {
        ...
        SQLStmt = "SELECT VERSION FROM APPLICATIONS WHERE ID = ?";
        m_version = new PreparedSentence(s, SQLStmt, SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE);
        SQLStmt = "SELECT VERSIONINT FROM APPLICATIONS WHERE ID = ?";
        m_versionint = new PreparedSentence(s, SQLStmt, SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE);
        SQLStmt = "SELECT * FROM PEOPLE WHERE 1 = 0";
        m_dummy = new StaticSentence(s, SQLStmt);
...
//*******************************************************************
        ...
        SQLStmt = "UPDATE PLACES SET X = ?, Y = ? WHERE ID = ?   ";
        m_updatePlaces = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.INT,
            Datas.INT,
            Datas.STRING
        }));
        SQLStmt = "SELECT ID FROM PRODUCTS " + 
                  "WHERE REFERENCE=? AND CODE=? AND NAME=? AND SITEGUID=?";
        m_getProductAllFields = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE REFERENCE=? AND CODE=? AND SITEGUID=?";
        m_getProductRefAndCode = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE REFERENCE=? AND NAME=? AND SITEGUID=?";
        m_getProductRefAndName = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE CODE=? AND NAME=? AND SITEGUID=?";
        m_getProductCodeAndName = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE REFERENCE=? AND SITEGUID=?";
        m_getProductByReference = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE CODE=? AND SITEGUID=?";
        m_getProductByCode = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT ID FROM PRODUCTS WHERE NAME=? AND SITEGUID=?";
        m_getProductByName = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}), productIdRead);
        SQLStmt = "SELECT COUNT(*) FROM TICKETLINES JOIN PAYMENTS ON TICKETLINES.TICKET = "
                + "PAYMENTS.RECEIPT JOIN RECEIPTS ON RECEIPTS.ID = TICKETLINES.TICKET WHERE  "
                + "RECEIPTS.MONEY=? AND TICKETLINES.TICKET =? ";
        m_getRecordCount = new PreparedSentence(s,SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}), SerializerReadInteger.INSTANCE);
        SQLStmt = "SELECT COUNT(*) FROM XSITELIST ";
        m_xsite = new PreparedSentence(s, SQLStmt, null, SerializerReadInteger.INSTANCE);
        //******************************************************************
        SQLStmt = "SELECT ID, NAME, APPPASSWORD, CARD, ROLE, IMAGE FROM PEOPLE P "
                + "JOIN SITEGUID S ON P.SITEGUID = S.GUID "
                + "WHERE S.GUID = P.SITEGUID AND VISIBLE = " + s.DB.TRUE()
                + " ORDER BY NAME";
        m_peoplevisible = new StaticSentence(s, SQLStmt, null, peopleread);
        SQLStmt = "SELECT ID, NAME, APPPASSWORD, CARD, ROLE, IMAGE FROM PEOPLE WHERE RIGHTSLEVEL > ? AND VISIBLE = " + s.DB.TRUE() + " ORDER BY NAME";
        m_peoplevisibleByRights = new StaticSentence(s, SQLStmt, SerializerWriteString.INSTANCE, peopleread);
        SQLStmt = "SELECT ID, NAME, APPPASSWORD, CARD, ROLE, IMAGE FROM PEOPLE WHERE CARD = ? AND VISIBLE = " + s.DB.TRUE();
        m_peoplebycard = new PreparedSentence(s, SQLStmt, SerializerWriteString.INSTANCE, peopleread);
        SQLStmt = "SELECT ID, NAME, APPPASSWORD, CARD, ROLE, IMAGE FROM PEOPLE WHERE NAME = 'SuperAdminUser' ";
        m_getsuperuser = new PreparedSentence(s, SQLStmt, null, peopleread);
        SQLStmt = "SELECT CONTENT FROM RESOURCES R JOIN SITEGUID S ON R.SITEGUID = S.GUID  WHERE NAME = ? AND S.GUID = R.SITEGUID ";
        m_resourcebytes = new PreparedSentence(s, SQLStmt, SerializerWriteString.INSTANCE, SerializerReadBytes.INSTANCE);
        Datas[] resourcedata = new Datas[]{Datas.STRING, Datas.STRING, Datas.INT, Datas.BYTES};
        SQLStmt = "INSERT INTO RESOURCES(ID, NAME, RESTYPE, CONTENT) VALUES (?, ?, ?, ?)";
        m_resourcebytesinsert = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(resourcedata));
        SQLStmt = "UPDATE RESOURCES SET NAME = ?, RESTYPE = ?, CONTENT = ? WHERE NAME = ?";
        m_resourcebytesupdate = new PreparedSentence(s, SQLStmt, new SerializerWriteBasicExt(resourcedata, new int[]{1, 2, 3, 1}));
        SQLStmt = "SELECT PERMISSIONS FROM ROLES WHERE ID = ? AND SITEGUID= ? ";
        m_rolepermissions = new PreparedSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING}), SerializerReadBytes.INSTANCE);
        SQLStmt = "UPDATE PEOPLE SET APPPASSWORD = ? WHERE ID = ?";
        m_changepassword = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{Datas.STRING, Datas.STRING}));
        SQLStmt = "SELECT MAX(HOSTSEQUENCE) FROM CLOSEDCASH WHERE HOST = ?";
        m_sequencecash = new StaticSentence(s,SQLStmt, SerializerWriteString.INSTANCE,SerializerReadInteger.INSTANCE);
        SQLStmt = "SELECT HOST, HOSTSEQUENCE, DATESTART, DATEEND, NOSALES FROM CLOSEDCASH WHERE MONEY = ?";
        m_activecash = new StaticSentence(s, SQLStmt, SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{
            Datas.STRING,
            Datas.INT,
            Datas.TIMESTAMP,
            Datas.TIMESTAMP,
            Datas.INT}));
        SQLStmt = "INSERT INTO CLOSEDCASH(MONEY, HOST, HOSTSEQUENCE, DATESTART, DATEEND) "
                + "VALUES (?, ?, ?, ?, ?)";
        m_insertcash = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING,
            Datas.INT,
            Datas.TIMESTAMP,
            Datas.TIMESTAMP}));
        SQLStmt = "INSERT INTO DRAWEROPENED (ID, NAME, TICKETID) "
                + "VALUES (?, ?, ?)";
        m_draweropened = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING,
            Datas.STRING}));
        SQLStmt = "INSERT INTO LINEREMOVED (ID,NAME, TICKETID, PRODUCTID, PRODUCTNAME, UNITS, VALUE) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        SQLStmt = "SELECT NAME FROM LOCATIONS WHERE ID = ?";
        m_locationfind = new StaticSentence(s, SQLStmt, SerializerWriteString.INSTANCE, SerializerReadString.INSTANCE);
//Add 13.2.14 JDL for new gui based permissions
        SQLStmt = "SELECT PERMISSIONS FROM PERMISSIONS WHERE ID = ?";
        m_permissionlist = new StaticSentence(s, SQLStmt, SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{
            Datas.STRING
        }));
        SQLStmt = "INSERT INTO PERMISSIONS (ID, PERMISSIONS) "
                + "VALUES (?, ?)";
        m_updatepermissions = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING}));
// added 1.3.14.14 JDL new routine to write to CSV table, to clean up CSV import routine
        SQLStmt = "INSERT INTO CSVIMPORT (ID, ROWNUMBER, CSVERROR, REFERENCE, CODE, NAME, PRICEBUY, PRICESELL, PREVIOUSBUY, PREVIOUSSELL, CATEGORY) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        m_insertCSVEntry = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING,
            Datas.STRING,
            Datas.STRING,
...
// added 17.12.2016 save import profile
        SQLStmt = "INSERT INTO CSVPROFILES (NAME, DETAILS) "
                + "VALUES (?, ?) ";
        m_insertCSVProfile = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.BYTES
        }));
// added 17.12.2016 update import profile
        SQLStmt = "UPDATE CSVPROFILES  SET DETAILS = ? WHERE NAME = ? "
                + "VALUES (?, ?) ";
        m_updateCSVProfile = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.BYTES
        }));
        SQLStmt = "SELECT DETAILS FROM CSVPROFILES WHERE NAME = ?";
        m_csvprofile = new StaticSentence(s, SQLStmt, SerializerWriteString.INSTANCE, new SerializerReadBasic(new Datas[]{
            Datas.BYTES}));
        SQLStmt = "INSERT INTO ORDERS (ID, ORDERID, QTY, DETAILS, ATTRIBUTES, NOTES, TICKETID, DISPLAYID, AUXILIARY, SEQUENCE) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";
        m_addOrder = new StaticSentence(s, SQLStmt, new SerializerWriteBasic(new Datas[]{
            Datas.STRING,
            Datas.STRING,
            Datas.INT,
            Datas.STRING,
    ...
    /**
     * @return (Integer)
     * @throws BasicException (BasicException)
     */
    ...
    public final void setResourceAsProperties(String sName, Properties p) {
        if (p == null) {
            setResource(sName, 0, null);
        } else {
            try {
                ByteArrayOutputStream o = new ByteArrayOutputStream();
                p.storeToXML(o, AppLocal.APP_NAME, "UTF8");
                setResource(sName, 0, o.toByteArray()); 
            } catch (IOException e) { 
            }
        }
    }
    ...
    /**
     * This is used by CSVimport to detect what type of product insert we are looking at,
